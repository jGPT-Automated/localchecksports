...
export function generateMetadata({ params }: { params: { city: string } }) {
  ...
  return {
    title: `Pickleball Courts in ${city} | LocalCheckSports`,
    description: `Find verified pickleball courts in ${city}. See court details, amenities, and live player activity through LocalCheck.`,
  };
}

export default function PickleballCityPage({ params }: { params: { city: string } }) {
  const displayCity = params.city
    .split('-')
    .map(part => part.charAt(0).toUpperCase() + part.slice(1))
    .join(' ');

  const courts = getCourtsBySport('pickleball', displayCity);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <nav className="flex gap-2 text-sm mb-8 text-court-600 dark:text-court-400">
        ...
        <span>/</span>
        <span className="text-court-900 dark:text-white">Pickleball in {displayCity}</span>
      </nav>

      <header className="mb-12">
        <h1 className="text-4xl font-bold text-court-900 dark:text-white mb-4">
          Pickleball Courts in {displayCity}
        </h1>
        <p className="text-lg text-court-600 dark:text-court-300 mb-4">
          {courts.length} verified pickleball court{courts.length !== 1 ? 's' : ''} ready to play
        </p>
      </header>
